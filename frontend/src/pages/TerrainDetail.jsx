...
export default function TerrainDetail() {
  ...
  if (!terrain) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-12 text-center">
        ...
        <Link to="/" className="btn-primary inline-block mt-4">Retour a l'accueil</Link>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <Link to="/" className="inline-flex items-center gap-2 text-gray-600 hover:text-orange-600 mb-6 transition-colors">
        <FiArrowLeft />
        Retour aux terrains
      </Link>
        ...
        <div className="p-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <div>
              ...
              <div className="flex items-center gap-1 text-gray-500 mt-1">
                <FiMapPin />
                <span>{terrain.address}, {terrain.neighborhood}, {terrain.city}</span>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
            <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-xl">
              ...
              <div>
                ...
                <div className="font-semibold">
                  {terrain.opening_time?.substring(0, 5)} - {terrain.closing_time?.substring(0, 5)}
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-xl">
              ...
              <div>
                <div className="text-sm text-gray-500">Gardien</div>
                <div className="font-semibold">{terrain.guardian_phone}</div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-xl">
              ...
              <div>
                <div className="text-sm text-gray-500">Orange Money</div>
                <div className="font-semibold">{terrain.orange_money_number}</div>
              </div>
            </div>

            {terrain.latitude && terrain.longitude && (
              <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-xl">
                <FiNavigation className="text-orange-600 text-xl" />
                <div>
                  <div className="text-sm text-gray-500">GPS</div>
                  <a
                    href={`https://www.google.com/maps?q=${terrain.latitude},${terrain.longitude}`}
                    target="_blank"
