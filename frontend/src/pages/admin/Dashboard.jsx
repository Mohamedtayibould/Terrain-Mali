...
import { fetchAdminStats, fetchAdminTerrains, deactivateTerrain } from '../../api/supabaseDirect';
import {
  FiPlus, FiEdit, FiTrash2, FiCalendar, FiDollarSign, FiMapPin,
  FiLoader, FiGrid, FiList, FiLogOut, FiHome,
...
export default function Dashboard() {
  ...
  const fetchAll = async () => {
    try {
      ...
      if (!ownerId) return;
      const [s, t] = await Promise.all([fetchAdminStats(ownerId), fetchAdminTerrains(ownerId)]);
      setStats(s);
      setTerrains(t);
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await deactivateTerrain(id, profile.id);
    ...
    <div className="flex h-screen bg-gray-900 overflow-hidden">
      <aside className="w-64 bg-gray-950 text-white flex flex-col fixed h-full z-10">
            ...
            return (
              <button key={item.id} onClick={() => setActiveTab(item.id)}
                ...
                {item.count !== undefined && (
                  <span className={`text-xs px-2 py-0.5 rounded-full ${activeTab === item.id ? 'bg-orange-700' : 'bg-gray-800'}`}>{item.count}</span>
                )}
              </button>
            );
          })}
        ...
        <div className="p-3 border-t border-gray-800 space-y-1">
          <a href="/" className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm text-gray-400 hover:bg-gray-800 hover:text-white">
            <FiHome size={18} /> Voir le site client
          </a>
          <button onClick={() => { logout(); window.location.href = '/'; }}
            className="w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm text-gray-400 hover:bg-red-900/30 hover:text-red-400">
            <FiLogOut size={18} /> Deconnexion
          </button>
        </div>
      </aside>
      ...
      <main className="flex-1 ml-64 p-6 overflow-y-auto">
        <div className="flex items-center justify-between mb-6">
          ...
          <Link to="/terrain/new" className="bg-orange-600 hover:bg-orange-700 text-white px-4 py-2 rounded-lg flex items-center gap-2 text-sm font-medium">
          ...
          <div className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden">
            {terrains.length === 0 ? (
              <div className="p-12 text-center">
                ...
                <Link to="/terrain/new" className="bg-orange-600 hover:bg-orange-700 text-white px-4 py-2 rounded-lg inline-flex items-center gap-2 text-sm font-medium mt-4">
                  <FiPlus /> Ajouter
                </Link>
              </div>
            ) : (
                ...
                <table className="w-full text-sm">
                  <thead className="bg-gray-900">
                    <tr>
                      ...
                      <th className="text-left px-5 py-3 font-medium text-gray-400">Ville</th>
                    ...
                    {terrains.map(t => (
                      <tr key={t.id} className="hover:bg-gray-750">
                        ...
                        <td className="px-5 py-4">
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${t.is_active ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
                            {t.is_active ? 'Actif' : 'Inactif'}
                          </span>
                        </td>
                        <td className="px-5 py-4">
                          <div className="flex justify-end gap-2">
                            <Link to={`/terrain/${t.id}/edit`} className="p-2 text-blue-400 hover:bg-blue-500/20 rounded-lg"><FiEdit size={16} /></Link>
                            <button onClick={() => handleDelete(t.id)} className="p-2 text-red-400 hover:bg-red-500/20 rounded-lg"><FiTrash2 size={16} /></button>
                          </div>
                        </td>
                      </tr>
                    ))}
        ...
        {activeTab === 'stats' && stats && (
          <div className="bg-gray-800 rounded-xl border border-gray-700 p-6">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center"><span className="text-gray-400">Terrains actifs</span><span className="text-white font-bold text-xl">{stats.terrains_count}</span></div>
              <div className="flex justify-between items-center"><span className="text-gray-400">Reservations totales</span><span className="text-white font-bold text-xl">{stats.reservations_count}</span></div>
              <div className="flex justify-between items-center"><span className="text-gray-400">Confirmees</span><span className="text-green-400 font-bold text-xl">{stats.confirmed_reservations}</span></div>
              <div className="border-t border-gray-700 pt-4 flex justify-between items-center"><span className="text-gray-400">Revenu total</span><span className="text-orange-400 font-bold text-2xl">{stats.total_revenue?.toLocaleString()} XOF</span></div>
            </div>
          </div>
        )}
      </main>
