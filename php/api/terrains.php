...
$uri = strtok($uri, '?');
$parts = array_filter(explode('/', preg_replace('#^/api/terrains#', '', $uri)));
$sub = $parts[0] ?? '';
...
if ($method === 'GET' && isset($parts[1]) && $sub !== 'cities') {
    $terrain_id = $sub;
    $slots_part = $parts[1] ?? '';
    if ($slots_part === 'slots') {
        $query = $_GET ?? [];
...
// GET /api/terrains/:id
if ($method === 'GET' && $sub !== '' && $sub !== 'cities' && !isset($parts[1])) {
...
if ($method === 'GET' && ($sub === '' || $sub === null)) {
    ...
    // Count total
    $count_endpoint = 'terrains?select=id&' . $filters;
    $count_data = supabase_get($count_endpoint);
